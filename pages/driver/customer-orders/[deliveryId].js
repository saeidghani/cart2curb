            ...
            <DriverPage title="Customer Orders">
                <div className="w-full py-3 flex flex-col space-y-4">
                    {(customerOrders?.products || [])?.map(product =>
                        <div key={product?._id} className="flex flex-col pb-4" style={{backgroundColor: 'rgba(114, 122, 139, 0.05)'}} >
                            {product?.productExtraInfo?.images?.length > 0 &&
                            <div className="flex items-center space-x-2 p-4"
                                 style={{borderBottom: '1px solid #D9D9D9'}}>
                                <img
                                    src={product?.productExtraInfo?.images[0]}
                                    alt="" width={56}/>
