                        ...
                        {props.addresses.map((item, index) => {
                            ...
                            return (
                                <Col xs={24} key={`address-${index}`}>
                                    <AddressCard title={`Address ${index + 1}`} value={address.join(', ')} disabled={props.addresses.filter(address => !deleted.includes(address._id)).length < 2} deleteHandler={() => deleteModal({
