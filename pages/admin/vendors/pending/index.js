...
const PendingDrivers = props => {
  ...
  const handleApprove = (vendorId) => {
    ...
    dispatch?.adminUser?.addPendingVendor({vendorId, body, token});
  };
  ...
  return (
    <AdminAuth>
      <Page title={false} headTitle={'Pending Vendors'} breadcrumb={breadcrumb}>
        {(pendingVendors || [])?.map(({store, vendor}) =>
          <Card className="mb-4 shadow pr-10">
            ...
            <div className="flex justify-end space-x-3 mt-5">
              <Button className="text-primary px-4" type="link" onClick={() => handleReject(vendor._id)}>Reject</Button>
              <Link href={{pathname: routes.admin.vendors.edit(vendor._id), query: {storeId: store?._id}}}>
                <Button
                  className="text-primary bg-transparent border border-solid border-primary px-8">Edit</Button>
              </Link>
              <Button className="text-white bg-primary px-4" onClick={() => handleApprove(vendor._id)}>Approve</Button>
            </div>
          </Card>
        )}
      </Page>
