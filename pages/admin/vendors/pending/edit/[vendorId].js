...
const EditPendingVendor = props => {
    ...
    const pendingVendorDetails = useSelector(state => state?.adminStore?.pendingVendor);
    ...
    const {editType, vendorId} = router.query;

    useEffect(() => {
        if (vendorId) {
            dispatch?.adminStore?.getPendingVendor(vendorId);
        }
    }, [vendorId]);
    ...
    useEffect(() => {
        ...
        const {store, vendor} = pendingVendorDetails || {};
...
export default EditPendingVendor;
